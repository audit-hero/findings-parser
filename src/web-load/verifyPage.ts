export const contentTooShort = (doc: string, contentLength = 30, wordCount = 5, linesCount = 3) => {
...
export const isNotFoundPage = (doc: string, title: string) => {
  let titleNotFound = title?.includes("404") || title?.toLowerCase().includes("not found")
  if (titleNotFound) return true
  ...
  if (bodyNotFound) return true

  return false
};

export const loading = (doc: string, loadingPhrases: string[]) => {
